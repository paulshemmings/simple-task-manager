...
app.controller('NotesController', function($scope, dataProvider) {
	...
	function initialize() {
		$scope.labels.getTopLabel(function() {
			if($scope.labels.top.length == 0) {
				/* add top
				$scope.labels.top .push({
					name: 'top',
					subLabels: []
				});
				// store top
				$scope.labels.persist();
				*/
			}	
		});
	}

	$scope.labels.select = function(node) {		
		$scope.labels.selected = node;
	};

	$scope.labels.rename = function() {
		$scope.labels.persist(function() {
			console.log('renamed label');
		});
	};

	$scope.labels.delete = function() {
		function recurse(list) {
			var index = list.indexOf($scope.labels.selected);
			if (index >-1) {
				list.splice(index, 1);
				$scope.labels.persist(function() {
			...
			for(var i = 0; i < list.length; i++) {
				recurse(list[i].subLabels);	
			}				
		}
		recurse($scope.labels.top);
	};

	$scope.labels.create = function() {
		$scope.labels.addSubLabel($scope.labels.selected, $scope.newLabel.name);
		$scope.labels.persist(function() {
			console.log('created label');
		});		
	}

	$scope.labels.erase = function() {
		dataProvider.deleteLabel($scope.labels.selected, function(){
			console.log('wiped');
		})
	}


	$scope.labels.getTopLabel = function(callback) {
		dataProvider.getTopLabel(function(data) {
			if (data.success) {
				if(data.content) {
					data.content.forEach(function(item) {
						$scope.labels.top.push(item);	
					})
    ...
    $scope.labels.addSubLabel = function(label, name) {
    ...
    $scope.labels.persist = function(callback) {   
    	// only ever persist the top node.
    	if(!$scope.labels.top) return;
        dataProvider.persistLabel($scope.labels.top[0], callback || function(response) {
        	console.log('persisted top node');
        });
    };
